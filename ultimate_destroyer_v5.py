"""
💥 ULTIMATE DESTROYER V6 — FIXED & ENHANCED
- Thread-safe with locks
- Fixed Telegram HTML formatting
- Improved ML score scaling
- Added API key security (optional)
- Better cooldown logic
- Optimized sleep loops
"""
...
# ══════════════════════════════════════════════════════════════
#  KuCoin Data Layer (unchanged, reliable)
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
#  Technical Indicators (35) – same as original, reliable
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
#  Multi-Agent AI System (unchanged logic, but thread-safe)
# ══════════════════════════════════════════════════════════════
class Agent:
...
class MLEngine:
    ...
    def predict(self,symbol)->dict:
        ...
        try:
            ...
            # FIXED SCORING: scale to 0-10 more reasonably
...
# ══════════════════════════════════════════════════════════════
#  Core Analysis Engine (thread-safe)
# ══════════════════════════════════════════════════════════════
def analyze_coin(symbol):
    ...
    # Scale final score to 0-10 (original was 0-10 anyway)
...
# ══════════════════════════════════════════════════════════════
#  Macro Guard & Telegram (HTML safe)
# ══════════════════════════════════════════════════════════════
...
def format_signal_msg(sig):
    ...
    lines=[
        f"<b>{emoji} {sig['type']} — {sig['coin']}/USDT</b>",
        "━━━━━━━━━━━━━━━━━━",
    ...
    lines+=["",f"⏰ {sig['timestamp'][:19].replace('T',' ')}","⚠️ <i>تحليل فني فقط</i>"]
    return '\n'.join(lines)

# ══════════════════════════════════════════════════════════════
#  Scan Loop (optimized sleep)
# ══════════════════════════════════════════════════════════════
def run_scan():
    with _state_lock:
        STATE['scan_count'] += 1
...
def scan_loop():
    while STATE['running']:
        ...
        # Optimized: sleep in one chunk
        time.sleep(interval_s)

# ── Whale Detector, Scalping, TP Tracker, Daily Report (unchanged but thread-safe) ──
...
def send_daily_report():
    ...
    lines=[f"☀️ <b>تقرير الصباح — Destroyer V5</b>",f"📅 {datetime.now().strftime('%Y-%m-%d')}","━━━━━━━━━━━━━━━━━━",
           f"₿ BTC: ${btc:,.0f} ({'+' if btc_chg>0 else ''}{btc_chg:.2f}%)",
           f"📊 Win Rate: {wr}% | ✅{p['win']} ❌{p['loss']}","","🔥 <b>أفضل 5 فرص الآن:</b>"]
    for i,s in enumerate(hot_sigs,1): lines.append(f"{i}. <b>{s['coin']}</b> — Score:{s['score']} | +{s['change24h']}%")
    ...
    lines+=["",f"🔴 HOT: {hot_count}",f"🟡 WARM: {warm_count}","━━━━━━━━━━━━━━━━━━","<i>Destroyer V5 | تحليل فني فقط</i> ⚠️"]
...
# ══════════════════════════════════════════════════════════════
#  Flask API Routes (with optional API key protection)
# ══════════════════════════════════════════════════════════════
...
def api_tg_test():
    ...
    ok=send_telegram('✅ <b>Destroyer V5</b> متصل!\n\nالبوت يعمل بنجاح 🚀',token,chat)
...
if __name__=='__main__':
    ...
    log.info('━'*50)
    log.info('💥 ULTIMATE DESTROYER V6 — يبدأ التشغيل (مصحح بالكامل)')
    ...
    if API_KEY:
        log.info('🔐 تم تفعيل حماية API Key (أرسل X-API-Key في الهيدر)')
    else:
        log.info('🔓 حماية API Key غير مفعلة (تعيين API_KEY لتفعيلها)')
